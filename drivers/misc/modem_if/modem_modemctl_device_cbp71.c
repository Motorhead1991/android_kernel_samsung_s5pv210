...
static irqreturn_t phone_active_irq_handler(int irq, void *_mc)
{
	...
	printk("[MODEM_IF] %s PHONE_ACTIVE level: %s\n", 	__func__, (gpio_get_value(GPIO_PHONE_ACTIVE) ? "HIGH" : "LOW "));
	#endif	
			...
			struct modem_data *pdata)
{
	...
	/*TODO: check*/
	//P8 version
	ret = request_irq(mc->irq_phone_active, phone_active_irq_handler, IRQ_TYPE_EDGE_BOTH, "phone_active", mc);
	...
	#ifdef MODEM_IF_DEBUG_STMTS
	else pr_err("[MODEM_IF] passed to irq_phone_active request_irq:%d return value:%d\n" , mc->irq_phone_active,ret);
	#endif	
	...
	#ifdef MODEM_IF_DEBUG_STMTS
	else pr_err("[MODEM_IF] passed to irq_phone_active enable_irq_wake:%d\n" , ret);
	#endif	
