...
/** OTA Poring - frame definitions
 */
/*TODO: check the CP buffer size...*/
#define CP_UPDATE_SIZE (0x4000 - 8) /* dpram size - mbx, magic*/
#define CP_UPDATE_FRAME_PAYLOAD (CP_UPDATE_SIZE - 10)
...
struct update_frame {
	...
	u16 current_frm;
	u16 len;
	u8 data[CP_UPDATE_FRAME_PAYLOAD];
	u8 crc;
	u8 end_idx;
	u8 __pad[1];
} __attribute__ ((packed));
...
static int io_dev_modem_update_write(struct io_device *iod, const char __user
	*buf, size_t count)
{
	struct sk_buff *skb;
	struct update_frame *frm;
	unsigned tfrm, cfrm, len, rest_len = count;
	char *cur_buf = (char *)buf;
	int err = -EFAULT;

	tfrm = count / CP_UPDATE_FRAME_PAYLOAD
			+ ((count % CP_UPDATE_FRAME_PAYLOAD) ? 1 : 0);

	pr_err("%s: img size = %d, total frame=%d\n", __func__, count, tfrm);

	#ifdef MODEM_IF_DEBUG_STMTS
	printk("[MODEM_IF] %s PHONE_ACTIVE level: %d\n", 	__func__, gpio_get_value(GPIO_PHONE_ACTIVE));
	printk("[MODEM_IF] %s GPIO_PHONE_ON_REV00 level: %d\n", 	__func__, gpio_get_value(GPIO_PHONE_ON_REV00));
	printk("[MODEM_IF] %s GPIO_PHONE_ON level: %d\n", 	__func__, gpio_get_value(GPIO_PHONE_ON));
    	printk("[MODEM_IF] %s GPIO_CP_RST level: %d\n", 	__func__, gpio_get_value(GPIO_CP_RST));
    	printk("[MODEM_IF] %s GPIO_PDA_ACTIVE level: %d\n", 	__func__, gpio_get_value(GPIO_PDA_ACTIVE));
    	printk("[MODEM_IF] %s GPIO_DPRAM_INT_N level: %d\n", 	__func__, gpio_get_value(GPIO_DPRAM_INT_N));
	#endif	


	/** TX framming loop */
	for (cfrm = 1; cfrm <= tfrm; cfrm++) {
		len = min((unsigned)rest_len, (unsigned)CP_UPDATE_FRAME_PAYLOAD);
		skb = alloc_skb(sizeof(struct update_frame), GFP_ATOMIC);
		if (!skb) {
			pr_err("[MODEM_IF] fail alloc skb (%d)\n", __LINE__);
			err = -ENOMEM;
			goto exit;
		}
		frm = (struct update_frame *)skb_put(skb,
						sizeof(struct update_frame));
		memset(frm, 0x00, sizeof(struct update_frame));
		frm->start_idx = HDLC_START;
		frm->total_frm = tfrm;
		frm->current_frm = cfrm;
		frm->len = len;

		err = copy_from_user(frm->data, cur_buf, len);
		if (err < 0) {
			pr_err("copy_from_user fail err = %d\n", err);
			goto exit;
		}
		if (len % 2) {
			pr_debug("add the 0xff for odd of last frame\n");
			memset(frm->data + len +1, 0xff, sizeof(char));
		}
		cur_buf += len;
		rest_len -= len;
		//frm->crc = make_crc(len, (u16 *)frm->data);
		frm->end_idx = HDLC_END;

		/*Write to Link device*/
		err = iod->link->send(iod->link, iod, skb);
		if (err < 0) {
			pr_err("[IOD] dpram write fail err = %d\n", err);
			goto exit;
		}
		else if(err==0)
		{
			pr_err("[MODEM_IF] %s total no. of transfered frames= %d out of %d\n",  __func__, cfrm, tfrm);
		}
	}
	/* send lask packet for SEND DONE */
	...
	if (err <0 ) {
        ...
        iod->link->clear_link_device_memory(iod->link);

	pr_debug("[MODEM_IF] Image Update done\n");
exit:
	return err;
}
...
static long misc_ioctl(struct file *filp, unsigned int cmd, unsigned long _arg)
{
	...
	case IOCTL_MODEM_STATUS:
		...
		if (p_state == STATE_NV_REBUILDING) {
			pr_info("[MODEM_IF] send nv rebuild state : %d\n", p_state);
	...
	case IOCTL_MODEM_GOTA_START:
		if (iod->link->ioctl)
			return iod->link->ioctl(iod->link, iod, cmd, _arg);
		pr_info("[MODEM_IF] link ioctl not assigned\n");
		return -ENODEV;
			...
			loff_t *f_pos)
{
	...
	/* when 'user request with count 0' means 'request per packet read'
	* return actual length of packet and ignore count value 0
	*/
	if (count && skb->len > count) {
